...
import { timeStampToDayNumber, timeStampToTime } from "./TimeJumper"
...
useEffect(
    ...
    */
    ...
    <label htmlFor="locationdropdown">Select Locations to view schedule</label>
        <select onChange={(evt) => {
        ...
        <option value={0} type="select" id="locationselect" className="form-control" required>Select a location </option>
            ...
            locations.map(
                (locations) => {
                    ...
                    return  <option key={`locationone-${locations.id}`} value={locations.id} > {locations.name} </option> 
    ...
    <label htmlFor="locationdropdown"></label>
       ...
       <h2>Below are the times you have previously been stopped at your selected TrainStop on a {CreateTextDayFromDayNumber(dateNumber)}:</h2>
       <div>
       {    stopsLocationOne.length?
                stopsLocationOne.map((stops) => {
                    return <div><b>{timeStampToTime(stops.stopTimestamp)} </b> <br></br>Notes: {stops.notes}</div>
                })
                : <div>There are no historical stops for this date at the selected location</div>
             }
